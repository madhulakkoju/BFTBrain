...
do
  ...
  tmux send-keys -t cloudlab:"$i" "cd ~/AP/BFTBrain/code && ../scripts/kill_process_port.sh $((9020+$count)) && ../scripts/kill_process_port.sh $((9020+$count+20))" C-m
done
...
do
  ...
  tmux send-keys -t cloudlab-learning:"$i" "cd ~/AP/BFTBrain/code/learning/ && python3 learning_agent.py -u $i -p $((9021+$i)) -n single" C-m
done
sleep 5
